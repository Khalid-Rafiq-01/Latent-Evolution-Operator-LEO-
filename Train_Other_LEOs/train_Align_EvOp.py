...
def get_data_loader(dataset, batch_size, shuffle=True):
    ...
    data = data[: len(data) - len(data) % batch_size]  # drop incomplete batch
    return DataLoader(
        data,
...
def log_val_examples(model, propagator, val_loader, device, epoch,
    ...
    for eg_id in range(num_examples):
        ...
        ax.set_title(
            f"Epoch {epoch} | λ_align={lambda_align:.3f} | "
...
def train():
    ...
    run = wandb.init(
        project="AE_Align_EvOp",
        config=dict(
            batch_size=128,
            lr=3e-4,
            epochs=150,
            lambda_align_max=1.0,
            warmup_frac=0.4,     # 40% epochs with λ_align = 0
            clip_norm=5.0,
        ),
    )
    config = wandb.config
    ...
    # VAE here is used as backbone: forward(x) -> (x_hat, z)
    ...
    # track best only AFTER alignment is active
    best_val_loss_aligned = float("inf")
    best_epoch_aligned = None
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        tr_RL1 = tr_RL2 = tr_ALIGN = tr_LOSS = 0.0
        n_tr = 0
        ...
        for X, X_tau, t_train, tau_train, Re_train in train_loader:
            ...
            loss = RL1 + RL2 + lam_align * ALIGN_L
            loss.backward()
        ...
        va_RL1 = va_RL2 = va_ALIGN = va_LOSS = 0.0
        n_va = 0
        ...
        with torch.no_grad():
            for X, X_tau, t_val, tau_val, Re_val in val_loader:
                ...
                vloss = RL1 + RL2 + lam_align * ALIGN_L
        ...
        log_val_examples(model, propagator, val_loader, DEVICE, epoch,
                         lambda_align=lam_align, num_examples=3)

        # Checkpoint logic
        # Only start choosing "best" once alignment is actually applied.
        if lam_align > 0.0:
            if va_LOSS < best_val_loss_aligned:
                best_val_loss_aligned = va_LOSS
                best_epoch_aligned = epoch
                torch.save(
                    {
                        "epoch": epoch,
                    ...
                    os.path.join(wandb.run.dir, "best_model_aligned.pt"),
                )

        if (epoch % 20 == 0) or (epoch == 1):
            print(
                f"[Epoch {epoch:03d}] "
                f"λ_align={lam_align:.3f} | "
                f"train: Loss={tr_LOSS:.3e} | RL1={tr_RL1:.3e} | RL2={tr_RL2:.3e} | CONS={tr_ALIGN:.3e} || "
                f"val: Loss={va_LOSS:.3e} | RL1={va_RL1:.3e} | RL2={va_RL2:.3e} | CONS={va_ALIGN:.3e}"
            )

    # record where best aligned model was
    if best_epoch_aligned is not None:
        wandb.summary["best_val_loss_aligned"] = best_val_loss_aligned
        wandb.summary["best_epoch_aligned"] = best_epoch_aligned
